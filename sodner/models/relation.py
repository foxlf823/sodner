...
from sodner.training.relation_metrics1 import RelationMetrics1
from sodner.models.entity_beam_pruner import Pruner
...
class RelationExtractor(Model):
    ...
    def _compute_relation_scores_biaffine(self, top_span_embeddings, top_span_mention_scores):
        ...
        for i in range(self._n_labels):
            AW = self._biaffine[i](top_span_embeddings)
            AWB = torch.matmul(AW, top_span_embeddings.transpose(1, 2))
            relation_scores_flat.append(AWB.unsqueeze(-1))
        relation_scores = torch.cat(relation_scores_flat, dim=3)
